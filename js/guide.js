...
brkn.Guide.prototype.enterDocument = function() {
  ...
  brkn.model.Controller.getInstance().subscribe(
  		brkn.model.Controller.Actions.TOGGLE_GUIDE, function(show) {
  ...
  minTime.add(new goog.date.Interval(
  		0, 0, 0, 0, -this.interval_ * (this.timeslots_ - 1)));
  ...
  channelNameStyle.type = 'text/css';
	...
	channelNameStyle.innerHTML = 'div#guide div.channels div.channel div.name' +
	  '{left:' + startTimeOffset + 'px !important}';
  ...
  dragger.defaultAction = function(x, y) {
	...
	this.getHandler()
		.listen(
				brkn.model.Clock.getInstance().clock,
				goog.Timer.TICK,
				goog.bind(function() {
					window.console.log()
